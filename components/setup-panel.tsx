...
export function SetupPanel({
  ...
  return (
    <section className="rounded-lg border border-border bg-panel p-5">
        ...
        {items.map((item) => (
          <div key={item.label} className="rounded-md border border-border p-4">
            <div className="flex items-center gap-2 text-sm font-medium">
              <CheckCircle2
                className={
                  item.ready ? "size-4 text-success" : "size-4 text-muted-foreground"
                }
                aria-hidden="true"
              />
              {item.label}
      ...
      {error ? (
        <pre className="mt-4 overflow-auto rounded-md bg-sidebar p-3 text-xs text-sidebar-foreground">
          {sanitizeSetupError(error)}
        </pre>
      ) : null}
    </section>
  );
...
function sanitizeSetupError(error: string) {
  if (/database|prisma|connect|econnrefused|timeout/i.test(error)) {
    return "Database connection failed. Check DATABASE_URL and run migrations.";
  }
  return "Setup check failed. Review the server logs for details.";
}
